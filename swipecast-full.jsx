...
.swipe-layout{display:grid;grid-template-columns:1fr 380px;gap:32px;min-height:70vh;}
...
@media(max-width:480px){.td-stats{grid-template-columns:1fr 1fr;gap:10px;}}
...
function TalentDashboard({session,myProfile,onNavigate,onViewCastingById,castingsVersion=0}){
  ...
  useEffect(()=>{loadApps();loadMessages();loadRecommended();loadSaved();loadRecentlyViewed();},[loadApps,loadMessages,loadRecommended,loadSaved,loadRecentlyViewed]);
  ...
  return(
    <div style={{maxWidth:1240,margin:"0 auto",padding:"24px 16px 64px"}}>
      ...
      <div style={{marginBottom:28}}>
        ...
        <h1 style={{fontWeight:800,fontSize:30,letterSpacing:-0.8,color:"var(--t1)",marginBottom:6,margin:"0 0 6px"}}>Welcome back, {firstName}. <span style={{fontSize:11,fontWeight:700,background:"rgba(26,26,46,0.08)",color:"var(--acc)",padding:"2px 8px",borderRadius:6,verticalAlign:"middle",marginLeft:4}}>MOBILE TALENT FIX TEST</span></h1>
              ...
              {appsLoading?(
                ...
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {filteredApps.map(app=>{
                    ...
                    return(
                      <div key={app.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 16px",background:"var(--bg)",border:"1px solid var(--bdr)",borderRadius:10,gap:12,flexWrap:"wrap",transition:"border-color .15s"}}>
                        <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                          <span style={{fontSize:12,fontWeight:600,padding:"4px 10px",borderRadius:6,background:st.bg,color:st.color,border:`1px solid ${st.color}33`}}>{st.label}</span>
                          {app.castings?.id&&<button className="btn-s btn-sm" style={{fontSize:11,padding:"4px 10px"}} onClick={()=>onViewCastingById?onViewCastingById(app.castings.id):onNavigate("search")}>View →</button>}
                        </div>
                      </div>
                    );
                  })}
                    ...
                    return(
                      <div key={c.id} style={{padding:"14px 16px",borderRadius:10,border:"1px solid var(--bdr)",background:"var(--bg)",display:"flex",flexDirection:"column",gap:6}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                          <div style={{fontWeight:700,fontSize:14,color:"var(--t1)",lineHeight:1.3}}>{c.title}</div>
                          <button
                            disabled={isSaving}
              ...
              {savedLoading?(
                ...
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {savedCastings.map(s=>{
                    ...
                    const dl=c?.deadline?fmtDeadline(c.deadline):null;
                    return(
                      <div key={s.casting_id} style={{padding:"12px 14px",borderRadius:10,border:"1px solid var(--bdr)",background:"var(--bg)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
            ...
            {rvLoading?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
                {recentlyViewed.map(r=>{
                  const c=r.castings;
                  return(
                    <div key={r.casting_id} style={{padding:"10px 12px",borderRadius:8,border:"1px solid var(--bdr)",background:"var(--bg)",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                      <div style={{minWidth:0}}>
  ...
  return(<div className="page page-wide">
    <div className="flex-between mb-20">
      <div>
        ...
        <h2 className="section-title" style={{marginBottom:0}}>{myProfile?.display_name||"Casting Director"}</h2>
      </div>
      <button className="btn-p" onClick={()=>setShowNew(true)}>+ New Casting</button>
    </div>
    ...
    {active&&activeRole?<>
      ...
      {folder==='pending'?<>
        ...
        <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
          <input className="input" style={{flex:1,minWidth:0,maxWidth:380}} placeholder={`Search in ${folder}…`} value={search} onChange={e=>setSearch(e.target.value)}/>
      ...
      {tab==="castings"?
        ...
        <div className="card-flat">{myCastings.map(c=>{
          ...
          return(<div key={c.id} className="casting-row" style={{cursor:"default",opacity:isRejected||isClosed||isArchived?0.7:1,flexDirection:"column",alignItems:"stretch",gap:0}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:12,width:"100%"}}>
              <div className="casting-row-left" style={{flex:1}}>
                <h4 style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>{c.title}{statusBadge}</h4>
                <p>{c.prod||"—"} · {c.location||"—"} · {(c.roles||[]).length} role{(c.roles||[]).length===1?"":"s"} · Deadline {c.deadline||"—"}{isPendingReview&&<em style={{color:"var(--t3)"}}> · Awaiting admin approval</em>}{isRejected&&<em style={{color:"#c0392b"}}> · Edit and resubmit below</em>}</p>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
