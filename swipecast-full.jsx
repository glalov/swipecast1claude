...
// ─── Selected-plan summary + activation. Shows plan, billing length, total
//     due. The "Activate" button calls the activate_membership SECURITY
//     DEFINER RPC. When real Stripe is wired this becomes a checkout redirect
//     and the RPC moves to a webhook handler — frontend stays the same.
function PlanSummaryPage({session,myProfile,planKey,onNavigate,onActivated,onReload}){
  ...
  const activate=async()=>{
    ...
    setBusy(true);setErr("");
    try{
      const {data,error}=await window.sb.rpc("activate_membership",{p_plan:plan.key});
      if(error)throw error;
      setDone(true);
      // Reload the local profile so the red banner disappears immediately
      // and the apply-gate flips to allow.
      if(typeof onReload==="function")onReload();
      // If the user came here from a casting they tried to apply to, return them.
      let returnTo=null;
      try{returnTo=JSON.parse(sessionStorage.getItem("sc_post_membership_return")||"null");}catch(_){}
      if(returnTo?.casting?.id){
        sessionStorage.removeItem("sc_post_membership_return");
        if(typeof onActivated==="function")onActivated(returnTo);
      }else{
        setTimeout(()=>onNavigate("search"),1200);
      }
    }catch(e){console.error("[membership] activate failed:",e);setErr(e.message||"Could not activate membership.");}
  ...
  return(<div className="page" style={{maxWidth:560,margin:"0 auto"}}>
    ...
    {done?<div className="success-msg" style={{background:"rgba(46,204,113,0.12)",border:"1px solid rgba(46,204,113,0.35)",borderRadius:14,padding:"32px 28px",textAlign:"center"}}>
      <div style={{fontSize:32,marginBottom:8}}>✓</div>
      <h3 style={{fontSize:20,fontWeight:800,marginBottom:8}}>Membership activated</h3>
      <p style={{color:"var(--t2)",fontSize:14}}>Your {plan.label} is live. Heading to your next step…</p>
    </div>:<>
      ...
      <button className="btn-p" style={{width:"100%",padding:"16px"}} onClick={activate} disabled={busy}>{busy?"Activating membership…":`Activate Membership · $${plan.total.toFixed(2)}`}</button>
      <p style={{fontSize:11,color:"var(--t3)",textAlign:"center",marginTop:12}}>Secure activation. Card collection via Stripe arrives next release — your plan will be confirmed under your email.</p>
    </>}
...
const STRIPE_ACTOR_LINK = "https://buy.stripe.com/test_bJe28l3C5bgq2XQcuL7g401";  // SlateCue Actor — 7-day free trial, then $9.99/mo (Stripe sandbox; swap for live link before launch)
