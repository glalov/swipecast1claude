...
function NewCastingModal({onClose,onPosted,uid,myProfile}){
  ...
  const [err,setErr]=useState("");
  const [busy,setBusy]=useState(false);
  // Form state is intentionally NOT reset on re-renders or prop changes — only on successful submit.
  const [f,setF]=useState({title:"",prod:"",type:"Film & TV",location:"",pay:"",union:"SAG-AFTRA",deadline:"",tagline:"",synopsis:""});
  const [roles,setRoles]=useState([{name:"",description:"",gender:"Any",age_range:"",ethnicity:"Any ethnicity",age_preset:"Any age",age_min:"",age_max:""}]);
  ...
  const addRole=()=>setRoles(p=>[...p,{name:"",description:"",gender:"Any",age_range:"",ethnicity:"Any ethnicity",age_preset:"Any age",age_min:"",age_max:""}]);
  ...
  return(<div className="modal-overlay" onClick={()=>!busy&&onClose()}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:700,maxHeight:"90vh",overflowY:"auto"}}>
    {step===2?<div className="success-msg">
      ...
      <button type="button" className="btn-p mt-20" onClick={()=>onPosted(inserted)}>Back to Dashboard</button>
    </div>:<>
