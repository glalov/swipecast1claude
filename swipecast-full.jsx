...
const BLOG_POSTS = [
  ...
  {id:5,title:"Building a Fairer Casting Ecosystem",date:"Mar 12, 2026",category:"Company",excerpt:"Our CEO on why SwipeCast replaced the $20-$30/month legacy-site tax with a single $9.99 plan, how we plan to sustain it, and the larger mission.",readTime:"6 min"},
];
...
const SUCCESS_STORIES = [
  ...
  {name:"Aisha Johnson",role:"National Nike Campaign",quote:"As a non-union actor just starting out, paying $20/month for casting sites was rough. Dropping to $9.99 on SwipeCast let me put the difference into headshots and classes instead.",img:TALENT[4].img},
];
...
const STRIPE_ACTOR_LINK = "https://buy.stripe.com/test_bJe28l3C5bgq2XQcuL7g401";  // SwipeCast Actor — 7-day free trial, then $9.99/mo (Stripe sandbox; swap for live link before launch)
...
function RegisterTalent({onNavigate}){
  ...
  const createAccount=async()=>{
    ...
    try{
      ...
      // Supabase quirk: if the email exists but is unconfirmed, signUp returns data with an identities array of length 0
...
function PricingPage({onNavigate}){
  ...
  return(<div className="page">
    <div className="info-hero"><div className="section-label">Pricing</div><h1>Simple pricing.<br/>No surprises.</h1><p>Talent: $9.99/month with a 7-day free trial. Casting directors pay just $20 per casting post — no subscriptions.</p></div>
    <div className="grid-3" style={{maxWidth:1000,margin:"0 auto"}}>{plans.map(p=>
      <div key={p.name} className="card" style={p.featured?{borderColor:"var(--acc)",background:"linear-gradient(165deg,var(--s1),rgba(26,26,46,.02))",position:"relative"}:{}}>
        ...
        <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:44,color:"var(--t1)",textAlign:"center"}}>{p.price}</div>
  ...
  return(<div className="page" style={{maxWidth:960}}>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"start",padding:"20px 0 60px"}}>
      ...
      <div style={{background:"var(--s1)",border:"1px solid var(--bdr)",borderRadius:16,padding:"32px 28px",position:"sticky",top:100}}>
        ...
        <p style={{color:"var(--t2)",fontSize:14,lineHeight:1.65,marginBottom:24}}>SwipeCast is <strong style={{color:"var(--t1)"}}>$9.99/month</strong> for talent after a 7-day free trial. Cancel anytime. You only need an account so casting directors can review your headshot and contact you.</p>
      ...
      <div>
        ...
        {mode==="signup"?<>
          ...
          <p style={{textAlign:"center",color:"var(--t3)",fontSize:12,marginTop:20}}>New to SwipeCast? <span onClick={()=>setMode("signup")} style={{color:"var(--blu)",cursor:"pointer",textDecoration:"underline"}}>Start your 7-day free trial</span></p>
        </>}
...
function Landing({onNavigate}){
  ...
  return(<>
    ...
    <section style={{padding:"72px 40px 40px",maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:"1.1fr 1fr",gap:60,alignItems:"center"}}><div>
      ...
      <p style={{color:"var(--t2)",fontSize:18,lineHeight:1.55,marginBottom:28,maxWidth:500}}>Every submission reviewed. No lost headshots. Just $9.99 a month. SwipeCast forces casting directors to see every actor — one at a time. Pass or callback. That's it.</p>
    ...
    <section style={{padding:"80px 40px",maxWidth:1200,margin:"0 auto"}}><div className="section-label">Pricing</div><div className="section-title">Simple pricing. No surprises.</div>
      <div className="grid-3">{[{name:"Talent",who:"Actors · Models · Performers",price:"$9.99",period:"per month · 7-day free trial",features:["Unlimited profile, photos & video","Unlimited submissions","Guaranteed views","Direct messages from casting","Audition scheduling","Cancel anytime"],featured:false,cta:"Start 7-Day Free Trial",action:"register-talent"},{name:"Casting Director",who:"Casting Directors · Producers",price:"$20",period:"per casting post",features:["Free account creation","Swipe-based review","Advanced talent filters","Callback management","Pay only when you post"],featured:true,cta:"Start Casting",action:"register-cd"},{name:"Studio",who:"Studios · Networks · Agencies",price:"Custom",period:"enterprise pricing",features:["Everything in Casting Director","Multi-user team access","Custom branding","API access","Dedicated account manager"],featured:false,cta:"Contact Sales",action:"contact"}].map(p=>
        <div key={p.name} className="card" style={p.featured?{borderColor:"var(--acc)",background:"linear-gradient(165deg,var(--s1),rgba(26,26,46,.02))",position:"relative"}:{}}>
          ...
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:44,color:"var(--t1)",textAlign:"center"}}>{p.price}</div><p style={{color:"var(--t2)",fontSize:12,marginBottom:24,textAlign:"center"}}>{p.period}</p>
          {p.features.map(f=><div key={f} style={{display:"flex",gap:8,alignItems:"center",padding:"7px 0",borderBottom:"1px solid var(--bdr)",fontSize:13,color:"var(--t2)"}}><span style={{color:"var(--grn)",fontWeight:700}}>✓</span>{f}</div>)}
    ...
    <section style={{padding:"40px 40px 60px",maxWidth:800,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {[
          {q:"How much does SwipeCast cost for actors?",a:"$9.99/month with a 7-day free trial. Cancel anytime. No 'featured' upgrade, no per-submission fees, no hidden tiers. Casting directors separately pay $20 per casting post."},
    ...
    <section style={{padding:"80px 40px",background:"linear-gradient(135deg,#1a1a2e 0%,#16213e 100%)",color:"#fff",textAlign:"center"}}>
      <div style={{maxWidth:640,margin:"0 auto"}}>
        ...
        <p style={{fontSize:16,lineHeight:1.6,color:"rgba(255,255,255,0.75)",marginBottom:32,maxWidth:500,margin:"0 auto 32px"}}>The first 5,000 actors and 500 casting directors get founding-member status — locked-in lifetime pricing, priority placement, and a badge on your profile.</p>
        {wlDone?
...
function MyProfilePage({session,profile,onReload,onNavigate}){
  ...
  const uploadResume=async(file)=>{
    ...
    if(!["pdf","doc","docx"].includes(ext)){setErr("Resume must be a PDF, DOC, or DOCX file.");return;}
    if(file.size>10*1024*1024){setErr("Resume must be under 10 MB.");return;}
    setUploading(true);setErr("");
    try{
      const path=`${session.user.id}/resume_${Date.now()}.${ext}`;
      const {error:upErr}=await window.sb.storage.from("reels").upload(path,file,{cacheControl:"3600",upsert:false,contentType:file.type||"application/pdf"});
      if(upErr)throw upErr;
      const {data:pub}=window.sb.storage.from("reels").getPublicUrl(path);
      await window.sb.from("profiles").update({resume_url:pub.publicUrl}).eq("id",session.user.id);
      onReload&&onReload();setMsg("Resume uploaded.");setTimeout(()=>setMsg(""),3000);
    }catch(e){setErr(e.message||"Upload failed. Make sure you've added the resume_url column — see the SQL in DEPLOY-NOW.");}finally{setUploading(false);}
  };
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const {data:sub}=window.sb.auth.onAuthStateChange(async(e,s)=>{
      setSession(s||null);
      if(s?.user)await loadProfile(s.user.id);else setMyProfile(null);
