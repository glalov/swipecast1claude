...
function SearchPage({onViewProfile,userType,onNavigate,onViewCasting,isLoggedIn,onRequireAuth,castingsVersion=0,session,myProfile}){
  ...
  const fetchTalent=useCallback(async()=>{
    try{
      const {data:ps,error}=await window.sb.from("profiles")
      ...
      setDbTalent(mapped);
    }catch(e){console.error("[SearchPage] talent fetch failed:",e);/* keep last good */}
  },[]);
...
export default function App(){
  ...
  return(
    <div className="app"><style>{css}</style>
      ...
      <main style={{flex:"1 1 auto",display:"flex",flexDirection:"column",minHeight:"calc(100vh - 80px)"}}>
        ...
        {/* SearchPage is kept mounted while on casting-detail so its fetched castings state
            survives the navigation. display:none hides it while keeping React state alive. */}
        ...
        {page==="dashboard"&&(!authReady?<PageLoader/>:<CDDashboard onViewProfile={viewProfile} onNavigate={navigate} session={session} myProfile={myProfile} castingsVersion={castingsVersion} bumpCastings={bumpCastings}/>)}
