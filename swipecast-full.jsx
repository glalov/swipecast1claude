  ...
  const loadClassInvitations=useCallback(async()=>{
    ...
    try{
      const{data,error}=await window.sb.from("class_invitations")
        .select("*,class:class_id(id,title,instructor_name,category,level,price,sale_price,image_url,start_date,format)")
        .eq("talent_user_id",uid)
        .neq("status","dismissed")
        .order("created_at",{ascending:false});
      if(error)throw error;
      setClassInvitations(data||[]);
    ...
    <div className="td-dash-outer">
      ...
      <div className="td-grid">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:24,minWidth:0}}>
          ...
          {/* Class Invitations — dashboard preview */}
          ...
          <div style={{background:"var(--s1)",border:"1px solid rgba(99,60,180,0.18)",borderRadius:14,overflow:"hidden"}}>
            ...
            <div style={{padding:24}}>
              ...
              ):classInvitations.length===0?null:(
                <>
                  <div style={{display:"flex",flexDirection:"column",gap:14}}>
                    {classInvitations.slice(0,3).map(inv=>{
                      const cls=inv.class;
                      return(<div key={inv.id} style={{background:"var(--bg)",border:"1px solid var(--bdr)",borderRadius:10,overflow:"hidden",display:"flex",flexDirection:"column"}}>
                        {cls?.image_url&&<div style={{height:80,background:`url(${cls.image_url}) center/cover no-repeat`,borderBottom:"1px solid var(--bdr)"}}/>}
                        <div style={{padding:"14px 16px"}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:6,marginBottom:4}}>
                            <div style={{fontWeight:700,fontSize:14,color:"var(--t1)",lineHeight:1.3}}>{cls?.title||"Class Invitation"}</div>
                            <button onClick={()=>dismissInvitation(inv.id)} title="Dismiss" style={{border:"none",background:"none",color:"var(--t3)",cursor:"pointer",fontSize:16,padding:0,lineHeight:1,flexShrink:0,marginTop:1}}>×</button>
                          </div>
                          <div style={{fontSize:11,color:"var(--t2)",marginBottom:4,display:"flex",flexWrap:"wrap",gap:"2px 8px"}}>
                            {cls?.instructor_name&&<span>with {cls.instructor_name}</span>}
                            {cls?.category&&<span style={{color:"var(--t3)"}}>· {cls.category}</span>}
                            {(cls?.sale_price||cls?.price)&&<span style={{color:"var(--t3)"}}>· {cls.sale_price||cls.price}</span>}
                          </div>
                          {inv.message&&<div style={{fontSize:11,color:"var(--t2)",background:"rgba(99,60,180,0.04)",border:"1px solid rgba(99,60,180,0.1)",borderRadius:6,padding:"7px 10px",marginBottom:10,lineHeight:1.5,fontStyle:"italic"}}>"{inv.message}"</div>}
                          <div style={{display:"flex",gap:8,marginTop:8}}>
                            ...
                            <button className="btn-s btn-sm" style={{fontSize:11}} onClick={()=>dismissInvitation(inv.id)}>Dismiss</button>
                          </div>
                        </div>
                      </div>);
                    })}
