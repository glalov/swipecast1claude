...
function TalentDashboard({session,myProfile,onNavigate,castingsVersion=0}){
  ...
  // Saved castings — placeholder until saved_castings table is added.
  // TODO: query saved_castings (talent_id, casting_id, saved_at) joined to castings.
  const [savedCastings]=useState([]);
  // Recently viewed — placeholder until recently_viewed_castings table is added.
  // TODO: track in DB table (talent_id, casting_id, viewed_at) OR localStorage array refreshed from DB.
  ...
  const loadRecommended=useCallback(async()=>{
    // TODO: implement profile-based matching using: location, gender, age_range, union_status, skills.
    // For now shows most recent open castings as a baseline.
    try{
    ...
    <div style={{maxWidth:1240,margin:"0 auto",padding:"32px 24px 64px"}}>
      ...
      <div className="td-grid">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:24,minWidth:0}}>
                    ...
                    return(
                      <div key={app.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 16px",background:"var(--bg)",border:"1px solid var(--bdr)",borderRadius:10,gap:12,flexWrap:"wrap",transition:"border-color .15s"}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                          ...
                          {app.castings?.slug&&<button className="btn-s btn-sm" style={{fontSize:11,padding:"4px 10px"}} onClick={()=>onNavigate("search")}>View →</button>}
                        </div>
                      </div>
                    );
                  })}
          ...
          <div style={{background:"var(--s1)",border:"1px solid var(--bdr)",borderRadius:14,overflow:"hidden"}}>
            <div style={{padding:"18px 24px",borderBottom:"1px solid var(--bdr)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
                ...
                <p style={{fontSize:11,color:"var(--t3)",margin:"2px 0 0"}}>Based on recent activity · profile matching coming soon</p>
              </div>
              ...
              {recsLoading?(
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  {recommended.map(c=>{
                    ...
                    return(
                      <div key={c.id} style={{padding:"14px 16px",borderRadius:10,border:"1px solid var(--bdr)",background:"var(--bg)",display:"flex",flexDirection:"column",gap:6,transition:"border-color .15s"}}>
                        <div style={{fontWeight:700,fontSize:13,color:"var(--t1)",lineHeight:1.3}}>{c.title}</div>
                        <div style={{fontSize:11,color:"var(--t2)"}}>{c.type}{c.location?` · ${c.location}`:""}</div>
                        {c.roles?.length>0&&<div style={{fontSize:11,color:"var(--t3)"}}>Roles: {c.roles.slice(0,2).map(r=>r.name).join(", ")}{c.roles.length>2?` +${c.roles.length-2}`:""}</div>}
                        {dl&&<div style={{fontSize:11,color:dl.urgent?"var(--red)":"var(--t3)",fontWeight:dl.urgent?600:400}}>Deadline: {dl.label}</div>}
                        <button className="btn-s btn-sm" style={{fontSize:11,marginTop:4,alignSelf:"flex-start"}} onClick={()=>onNavigate("search")}>View Casting →</button>
                      </div>
                    );
                  })}
                </div>
          ...
          {/* Saved Castings */}
          {/* TODO: query saved_castings table (talent_id uuid, casting_id uuid, saved_at timestamptz) joined to castings.
              Add a bookmark icon on CastingDetailPage that inserts/deletes a saved_castings row.
              RLS: talent can only read/write their own rows. */}
          <div style={{background:"var(--s1)",border:"1px solid var(--bdr)",borderRadius:14,overflow:"hidden"}}>
            <div style={{padding:"18px 24px",borderBottom:"1px solid var(--bdr)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <div style={{padding:24}}>
              {savedCastings.length===0?(
                ...
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {savedCastings.map(c=>(
                    <div key={c.id} style={{padding:"12px 14px",borderRadius:10,border:"1px solid var(--bdr)",background:"var(--bg)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                      <div style={{minWidth:0}}>
                        <div style={{fontWeight:600,fontSize:13,color:"var(--t1)"}}>{c.title}</div>
                        <div style={{fontSize:11,color:"var(--t3)"}}>{c.type}{c.location?` · ${c.location}`:""}</div>
                      </div>
                      <button className="btn-s btn-sm" style={{fontSize:11,flexShrink:0}} onClick={()=>onNavigate("search")}>View →</button>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          ...
          {/* TODO: implement — needs a recently_viewed_castings table (id, talent_id uuid, casting_id uuid, viewed_at timestamptz)
              OR a localStorage array of casting IDs synced to DB. Update on CastingDetailPage mount.
            ...
            {recentlyViewed.length===0?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
                {recentlyViewed.map(c=>(
                  <div key={c.id} style={{padding:"10px 12px",borderRadius:8,border:"1px solid var(--bdr)",background:"var(--bg)",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                    <div style={{minWidth:0}}>
                      <div style={{fontWeight:600,fontSize:12,color:"var(--t1)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.title}</div>
                      <div style={{fontSize:11,color:"var(--t3)"}}>{c.type}{c.location?` · ${c.location}`:""}</div>
                    </div>
                    <button className="btn-s btn-sm" style={{fontSize:10,padding:"3px 8px",flexShrink:0}} onClick={()=>onNavigate("search")}>View</button>
                  </div>
                ))}
              </div>
            )}
