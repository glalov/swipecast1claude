...
function NewCastingModal({onClose,onPosted,uid,myProfile}){
  ...
  const BLANK_F={title:"",prod:"",type:"Film & TV",location:"",pay:"",union:"SAG-AFTRA",deadline:"",tagline:"",synopsis:""};
  ...
  const submit=async(e)=>{
    ...
    setBusy(true);
    try{
      const payload={cd_id:uid,title:f.title.trim(),type:f.type,prod:f.prod||null,tagline:f.tagline||null,synopsis:f.synopsis||null,location:f.location||null,pay:f.pay||null,union_status:f.union,deadline:f.deadline||null,status:"pending_review",published:false};
  ...
  return(<div className="modal-overlay" onClick={handleClose}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:700,maxHeight:"90vh",overflowY:"auto"}}>
    {step===2?<div className="success-msg">
      ...
      <div style={{display:"flex",gap:12,marginTop:24}}>
        <button type="button" className="btn-p" style={{flex:1}} onClick={submit} disabled={busy}>{busy?"Submitting…":"Submit for Review"}</button>
        <button type="button" className="btn-s" onClick={handleClose} disabled={busy}>Cancel</button>
      </div>
    </>}
  </div></div>);
}
