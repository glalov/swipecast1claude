...
function RegisterCD({onNavigate}){
  ...
  return(
    <div className="page"><div style={{maxWidth:640,margin:"0 auto"}}>
      ...
      <label className="checkbox-row" style={{marginTop:16}}><input type="checkbox" checked={f.agree} onChange={e=>up("agree",e.target.checked)}/> I agree to SwipeCast's Terms of Service and Privacy Policy</label>
...
function SearchPage({onViewProfile,userType,onNavigate,onViewCasting,isLoggedIn,onRequireAuth}){
  ...
  useEffect(()=>{(async()=>{
    try{
      const {data:cs}=await window.sb.from("castings").select("*,roles(id,name,description,gender,age_range,ethnicity,pay)").eq("status","open").eq("published",true).order("created_at",{ascending:false});
      // normalize DB castings to match demo shape so the rest of the UI just works
    ...
    try{
      const {data:ps}=await window.sb.from("profiles").select("id,display_name,headshot_url,age,gender,ethnicity,location,union_status,skills,bio").eq("user_type","talent").eq("visible",true).eq("suspended",false).eq("onboarded",true).limit(200);
    ...
    {active&&activeRole?<>
      ...
      {tab==="castings"?
        myCastings.length===0?<div className="card" style={{textAlign:"center",padding:48}}>
          <p style={{color:"var(--t3)",marginBottom:16}}>You haven't posted any castings yet.</p>
          <button className="btn-p" onClick={()=>setShowNew(true)}>+ Post Your First Casting</button>
        </div>:
...
function NewCastingModal({onClose,onPosted,uid}){
  ...
  return(<div className="modal-overlay" onClick={()=>!busy&&onClose()}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:700,maxHeight:"90vh",overflowY:"auto"}}>
    {step===2?<div className="success-msg">
      ...
      <p>Your casting is now live and visible to every actor on SwipeCast. They'll get notified and start submitting immediately.</p>
      <button className="btn-p mt-20" onClick={onPosted}>Back to Dashboard</button>
    </div>:<>
      <h2>Post New Casting</h2>
      <p style={{color:"var(--t2)",fontSize:13,marginTop:-8,marginBottom:20}}>Free during launch. Goes live immediately.</p>
      {err&&<div style={{background:"rgba(255,100,100,0.1)",border:"1px solid rgba(255,100,100,0.3)",color:"#c0392b",padding:"10px 14px",borderRadius:8,fontSize:13,marginBottom:16}}>{err}</div>}
...
function MyProfilePage({session,profile,onReload,onNavigate}){
  ...
  const uploadFile=async(file,bucket,field)=>{
    ...
    try{
      ...
      const {error:upErr}=await window.sb.storage.from(bucket).upload(path,file,{cacheControl:"3600",upsert:false});
...
function AdminPage({session,onNavigate}){
  ...
  return(<div className="page page-wide">
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
      <div style={{background:"var(--acc)",color:"#fff",padding:"4px 10px",borderRadius:100,fontSize:10,fontWeight:800,letterSpacing:1.5,fontFamily:"'Syne',sans-serif"}}>OWNER / ADMIN</div>
    </div>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const {data:sub}=window.sb.auth.onAuthStateChange(async(_e,s)=>{setSession(s||null);if(s?.user)await loadProfile(s.user.id);else setMyProfile(null);});
  ...
  const completeAuth=()=>{if(pendingApply){const c=pendingApply.casting;setViewingCasting(c);window.scrollTo(0,0);setPage("casting-detail");}else{setPage("home");}};
  ...
  const signOut=async()=>{await window.sb.auth.signOut();setMyProfile(null);navigate("home");};
