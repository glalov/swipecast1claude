  ...
  const fetchCastings=useCallback(async()=>{
    ...
    try{
      ...
      const {data,error}=await Promise.race([
        window.sb.from("castings")
          .select("id,title,type,prod,tagline,synopsis,location,pay,deadline,union_status,featured,cd_id,roles(id,name,description,gender,age_range,ethnicity,pay),profiles:cd_id(display_name,company_name,headshot_url,verified,identity_verified,background_check_status,can_post_castings,verification_status)")
          .eq("status","open").eq("published",true)
          .order("featured",{ascending:false})
          .order("created_at",{ascending:false})
          .limit(12),
  ...
  const viewCastingById=useCallback(async(castingId)=>{
    ...
    try{
      ...
      const c={
        ...
        cd_id:data.cd_id,_cd:data.profiles||null,
        roles:(data.roles||[]).map(r=>({
          name:r.name||"",desc:r.description||"",type:"Supporting",
