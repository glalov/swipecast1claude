  ...
  <script>
    ...
    (function(){{
      try{{
        ...
        if(prev&&prev!==B){{
          /* Reload at most once per build per session — prevents an infinite
             reload loop if two CDN edges briefly serve different builds. */
          if(sessionStorage.getItem("sc_reload_build")!==B){{
            sessionStorage.setItem("sc_reload_build",B);
            window.location.reload();
          }}
        }}
      }}catch(e){{/* storage blocked (private mode / disabled) — never block boot */}}
    }})();
  </script>
  <style>
    ...
    #cs-loading .logo{{color:#fff;font-size:22px;font-weight:800;font-family:-apple-system,sans-serif;letter-spacing:-0.5px;display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;gap:10px;}}
    #cs-loading .logo-i{{width:36px;height:36px;background:#fff;border-radius:8px;display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;-webkit-justify-content:center;justify-content:center;flex-shrink:0;}}
    #cs-error{{
    ...
    /* Intro reveal — logo eases out of black, then the site is revealed.
       Plays on every page load (refresh, address-bar entry, new tab); the
       reduced-motion guard in the inline script below skips it for users who
       prefer reduced motion. */
    #cs-intro{{
      position:fixed;top:0;right:0;bottom:0;left:0;z-index:100000;overflow:hidden;
    ...
    @keyframes cs-bg-down{{to{{-webkit-transform:translateY(101%);transform:translateY(101%);}}}}
    @-webkit-keyframes cs-mark-up{{to{{-webkit-transform:translateY(-101%);transform:translateY(-101%);}}}}
...
<body>
  ...
  <script>
    ...
    (function(){{
      ...
      /* The flash is armed off cs-ready, not off t0, so it always sits the same
         distance after the logo appears no matter when the first frame landed.
         The t0 backstop covers the 400ms fallback path below, and the `flashed`
         guard means whichever fires first wins. It is a no-op once the curtain
         has gone, so a fast boot can never flash into an empty screen. */
      var flashed=false;
      function csFlash(){{ if(flashed||gone)return; flashed=true; el.classList.add('cs-flash'); }}
      function csReady(){{
        if(el.classList.contains('cs-ready'))return;
        el.classList.add('cs-ready');
        setTimeout(csFlash,1000);
      }}
      raf(function(){{raf(csReady);}});
  ...
  <noscript>
    <div style="font-family:sans-serif;max-width:800px;margin:40px auto;padding:0 24px;color:#1B1C20;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:32px;">
        <div style="width:40px;height:40px;background:#1A1A2E;border-radius:10px;display:flex;align-items:center;justify-content:center;">
      ...
      <p style="font-size:16px;line-height:1.7;margin:0 0 24px;color:#444;">
        ...
        Free forever for actors. Premium plans from $10.75/month for unlimited submissions.
  ...
  <script>
    /* ── Runtime error infrastructure ────────────────────────────────────────
       The crash screen means "the app did NOT start" — NOT merely "an error
       fired." A single stray error (browser extension, injected assistant
       script, a cross-origin "Script error.", a benign ResizeObserver loop)
       must never blank an app that was about to render fine. So error handlers
       only LOG; a polling verify-mount check is the ONLY thing that shows the
       crash screen, and only when React genuinely rendered nothing in time.
       Errors AFTER React mounts are caught per-section by ErrorBoundary.
    ──────────────────────────────────────────────────────────────────────── */
    ...
    function csShowCrash(msg){{
      var ld=document.getElementById('cs-loading'); if(ld) ld.style.display='none';
      var er=document.getElementById('cs-error');   if(er) er.style.display='flex';
      if(showDebug && msg){{
    ...
    /* Noise we never count as a real failure (browser/extension chatter). */
    function csIsBenign(msg){{ return /ResizeObserver loop|^Script error\\.?$/i.test(msg||''); }}

    /* Log uncaught errors for diagnostics only — do NOT show the crash screen
       here. The verify-mount poll below decides whether the app truly failed. */
    window.addEventListener('error', function(e){{
      var msg = (e.message||'unknown') + '\\n' + (e.filename||'?') + ':' + (e.lineno||0);
      if(!csIsBenign(e.message)) window.__SC_ERR.push({{type:'error', msg:msg, ts:new Date().toISOString()}});
    ...
    /* Verify-mount — the ONLY path to the crash screen. Poll until React mounts
       (flag set by App's first effect, or #root has painted children). Show the
       crash UI only if nothing rendered within the grace window — a genuine
       startup failure, never a stray error on an otherwise-healthy load.
       30s grace is generous for slow mobile networks; a fast boot clears it
       within a couple hundred ms. */
    (function(){{
      var DEADLINE = Date.now() + 30000;
      (function poll(){{
        if(window.__CS_REACT_MOUNTED) return;
        var root=document.getElementById('root');
        if(root && root.childElementCount>0){{ window.__CS_REACT_MOUNTED=true; return; }}
        if(Date.now()>=DEADLINE){{
          ...
          csShowCrash('App did not start within '+elapsed+'s.\\n'+(errList||'No JS errors captured — likely a network issue.'));
          return;
        }}
        setTimeout(poll, 400);
      }})();
...
{app_tag}
  ...
  (function(){{
    ...
    window.__CS_LOAD_CASTORIA=load;

    /* ── Launcher stub ───────────────────────────────────────────────────
       castoria.js is 161KB br and loads lazily behind app.js (744KB br), so
       on a slow connection it can arrive 10s+ in — and the launcher's drop
       was inheriting that, which is why it sometimes appeared on time and
       sometimes long after the page. This is a few hundred bytes of inline
       button that drops on schedule no matter what the network is doing.
       The real widget removes it the moment it mounts. */
    (function(){{
      var b=document.createElement('button');
      b.id='cs-stub-launch';
      b.type='button';
      b.setAttribute('aria-label','Message Super Assistant');
      b.style.cssText='position:fixed;right:22px;bottom:22px;width:58px;height:58px;border:0;'
        +'border-radius:50%;cursor:pointer;overflow:hidden;display:grid;place-items:center;padding:0;'
        +'z-index:2147482999;visibility:hidden;opacity:0;'
        +'background-image:linear-gradient(150deg,#FFFFFF,#EDEDF1 52%,#C8C8D0);'
        +'box-shadow:inset 0 0 0 1px rgba(19,19,24,.16),0 8px 26px rgba(0,0,0,.24);';
      b.innerHTML='<svg viewBox="0 0 32 32" width="58" height="58" aria-hidden="true" style="display:block">'
        +'<rect x="4" y="12.6" width="24" height="14.4" rx="2.6" fill="#131318"/>'
        +'<rect x="7" y="16" width="18" height="2" rx="1" fill="rgba(255,255,255,.34)"/>'
        +'<rect x="7" y="20" width="12" height="2" rx="1" fill="rgba(255,255,255,.20)"/>'
        +'<rect x="3.5" y="5" width="25" height="6.6" rx="1.4" fill="#131318"/>'
        +'<g clip-path="url(#csStubClip)" fill="#FFFFFF">'
        +'<polygon points="5,11.6 8.6,5 12,5 8.4,11.6"/><polygon points="13,11.6 16.6,5 20,5 16.4,11.6"/>'
        +'<polygon points="21,11.6 24.6,5 28,5 24.4,11.6"/></g>'
        +'<defs><clipPath id="csStubClip"><rect x="3.5" y="5" width="25" height="6.6" rx="1.4"/></clipPath></defs>'
        +'</svg>';
      var st=document.createElement('style');
      st.textContent='#cs-stub-launch.drop{{visibility:visible;opacity:1;'
        +'animation:csStubDrop 1.05s backwards}}'
        +'#cs-stub-launch.drop svg{{animation:csStubSquash 1.05s backwards}}'
        +'@keyframes csStubDrop{{'
        ...
        +'95%{{transform:translateY(0)}}98%{{transform:translateY(-2px)}}100%{{transform:translateY(0)}}}}'
        +'@keyframes csStubSquash{{0%,40%{{transform:scale(1,1)}}47%{{transform:scale(1.2,.8)}}'
        +'54%{{transform:scale(.95,1.05)}}62%,74%{{transform:scale(1,1)}}78%{{transform:scale(1.1,.9)}}'
        +'84%,100%{{transform:scale(1,1)}}}}'
        +'@media(prefers-reduced-motion:reduce){{#cs-stub-launch.drop,#cs-stub-launch.drop svg{{animation:none}}}}';
      document.head.appendChild(st);
      document.body.appendChild(b);

      var dropped=false;
      function drop(){{ if(dropped)return; dropped=true; b.classList.add('drop'); }}
      function schedule(){{
        var at=window.__CS_INTRO_DONE_AT||Date.now();
        setTimeout(drop,Math.max(0,1500-(Date.now()-at)));
      }}
      if(window.__CS_INTRO_DONE)schedule();
      else window.addEventListener('cs:intro-done',schedule,{{once:true}});
      setTimeout(drop,8000);
      /* Tapping before the real widget exists must still open it: flag the
         intent - castoria honours __CS_CASTORIA_OPEN_NOW on mount - and stop
         waiting for idle. */
      b.addEventListener('click',function(){{
        window.__CS_CASTORIA_OPEN_NOW=true;
