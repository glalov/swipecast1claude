...
const OPENING_FREE=[
 ...
 'Who are the biggest talent agencies?',
...
function suggPlus(list,cap){
  ...
  /* Stride rather than slice. A contiguous run came back as eleven craft
     lessons in a row; stepping through the pool spreads the fill across
     craft, auditions, materials, the industry and the platform. */
  const need=Math.max(0,cap-list.length), fill=[], taken=new Set();
  let at=poolAt%pool.length;
  for(let i=0;fill.length<need&&i<pool.length*2;i++){
    const idx=(at+i*7)%pool.length;
    if(taken.has(idx))continue;
    taken.add(idx); fill.push(pool[idx]);
  }
  poolAt+=3;
  sugg(list.concat(fill));
}
...
function refreshSuggHint(){
  ...
  if(!w||!wrap)return;
  refreshThreadEdge();
  const scrolls=w.scrollHeight>w.clientHeight+4;
  const atEnd=(w.scrollTop+w.clientHeight)>=(w.scrollHeight-4);
  wrap.classList.toggle('scrolls',scrolls);
  wrap.classList.toggle('atend',atEnd);
}
function sugg(list){
  const w=$('sugg');w.innerHTML='';w.scrollTop=0;
