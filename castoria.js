...
/* The nudge shows once per visitor and never competes with an open panel. */
const NUDGE_KEY='cs_sa_nudge_seen';
function hideNudge(){
...
function armNudge(){
  ...
  let seen=false;
  try{seen=localStorage.getItem(NUDGE_KEY)==='1';}catch(_){}
  if(seen)return;
  setTimeout(()=>{
    if(st.open)return;
