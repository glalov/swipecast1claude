...
const st={plan:'visitor',level:'beginner',open:false,misses:0,lastOut:null,lesson:null,step:0,offer:null,paused:null,identity:null,asked:new Set(),moreQueue:[],profile:{weak:[],declared:null,taught:[]},lastConcept:null};
...
function suggBtn(label){
  ...
  b.onclick=()=>{$('sugg').innerHTML='';st.moreQueue=[];send(label);};
  return b;
}
...
function wireTools(){
  ...
  if(tM2)tM2.onclick=function(){
    /* Hand out the next batch before scrolling, so the button keeps revealing
       ...
       at whatever happened to be rendered. */
    if(st.moreQueue&&st.moreQueue.length)suggAppend(st.moreQueue.splice(0,8));
