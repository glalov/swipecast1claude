...
THE MONOLOGUE MISTAKE BEGINNERS MAKE
Never let a beginner make their main monologue the heaviest thing they own -
the screaming one, the sobbing one, the violent one. Explain it from the
casting director's side of the room: they have watched thousands of auditions,
several of today's have already been someone crying or yelling at them from
four feet away, and nobody enjoys being screamed at in a small room. Actors
reach for it because big emotion feels like proof of range and suffering feels
powerful; for a first impression it backfires and they get remembered as the
person who made the room heavy. Tell them to lead with something LIGHT, ideally
funny - making a tired panel laugh is rarer and harder than a good dramatic
piece, and it is genuinely memorable. Keep it inside their real age range and
inside something they could be cast as tomorrow. The heavy piece is the SECOND
of the contrasting pair, offered only if asked. Say plainly that this does not
mean they cannot do drama; it means they do not open with it.
...
const SMALL=[
 {id:'greet',re:/^(hi|hey+|hello+|yo|hiya|heya|sup|howdy|hi there|hey there|good (morning|afternoon|evening))[\s!.,?]*$/i,
  ...
  s:['What can I ask you?','How do I improve my profile?','What does Premium include?']},
 ...
 /* Swearing on its own is a mood, not a question. Kept AFTER 'rude' so an
    insult aimed at the assistant still gets the rude reply, and length-capped
    so "how the fuck do I get an agent" still routes to the agent answer. */
 {id:'vent',
  re:/^(?=[\s\S]{0,44}$)\W*(?:(?:oh|omg|god|man|dude|well|just|so|ok|okay|ffs)\W+)*(?:f+u+c+k+|sh+i+t+|shite|damn|goddamn|crap|bollocks|bugger|wank|arse|bullshit|wtf|omfg|fml|ffs|ugh+|argh+|aargh+)\w*(?:\W+(?:this|that|it|off|sake|hell|man|dude|god|omg|seriously|again|everything|all|me|my|life|thing|things))*\W*$/i,
  a:c=>`Yeah — something's clearly gone wrong.`,
  b:c=>`Tell me what happened in your own words. Swear all you like, it doesn't bother me — I'll either sort it or put a person on it.`,
  s:['Talk to a human','Why is nobody getting back to me?','I have a billing problem']},
...
{id:'joinunion',k:'do i need to join should i join when to join union sag aftra equity eligible eligibility vouchers taft hartley initiation fee must pay state financial core join the union',
 a:c=>`<b>No — you do not need to join to work, and for most actors the question answers itself, because you cannot join until somebody wants you enough to make you eligible.</b>`+
 `<div class="hd">How eligibility actually happens</div><ul><li><b>You get cast in a principal role on a union production.</b> The production Taft-Hartleys you — that is the common route, and it starts with being right for a part, not with paperwork</li><li><b>Background vouchers.</b> A set number of days worked as a union background performer makes you eligible. Slow, real, and the reason some actors do background work at all</li><li><b>A sister union.</b> Membership and qualifying work in an affiliated performers' union can make you eligible</li></ul>`+
 `<p>Exact thresholds, fees and rules change, and they are not the same in every state. <b>Check the current terms with SAG-AFTRA (or Equity for stage) directly</b> before you act on anything — including anything I tell you.</p>`+
 `<div class="hd">The timing decision</div><ul><li>Joining is <b>not a career stage you graduate into</b>. It is a trade: protection on union work, in exchange for no longer taking non-union work under that union's jurisdiction</li><li>If almost everything you are being offered is non-union, joining early <b>shrinks the pool of jobs you are allowed to accept</b> — that is the real cost, not the fee</li><li>When union work is what keeps arriving, join. Being eligible and waiting is a normal, sensible position to sit in for a while</li><li>The initiation fee is <b>substantial</b> and dues are ongoing. Know the number before you are asked for it</li></ul>`+
 `<p>What the union does once you are in it — minimums, residuals, health and pension, someone to call — is a separate answer. Ask me what the union actually does.</p>`+
 `<p>On CastSlate every casting shows its union status and you can filter by it, so you can see what your market is actually offering before you decide.</p>`,
 src:['safety'],s:['What does the union actually do?','What is Taft-Hartley?','Is background work worth it?']},
...
{id:'approachagency',k:'approach agency agencies query letter mailing submission package unsolicited target tier follow up referral cover email who do i send to how do i approach an agency contact agents',
 a:c=>`Getting an agent and approaching one are two different problems. This is the second one: you have the materials, now how do you actually make contact without being deleted.`+
 `<div class="hd">Target before you send</div><ul><li><b>Check their own site first.</b> Plenty of companies say "no unsolicited submissions" in plain English. Sending anyway is not persistence, it is the reason your email gets a rule made for it</li><li><b>Match the tier to where you actually are.</b> The largest agencies sign almost entirely from referrals, showcases and existing heat. Boutique and mid-size offices genuinely read submissions and genuinely sign new people</li><li><b>Name a person.</b> "Dear Sir/Madam" and a BCC field to forty agencies both read as a mailout, and neither gets opened twice</li></ul>`+
 `<div class="hd">What to send</div><ul><li>A subject line that says what you are: type, market, union status</li><li><b>Three sentences.</b> Who you are, what you are right for, and what is new — a booking, a new reel, a course you just finished. Nothing about your childhood love of performing</li><li>Something that <b>opens in one click</b>. An attachment nobody downloads is the same as no footage at all</li><li>Current headshot and resume, and nothing else. No life story, no PDF deck</li></ul>`+
 `<div class="hd">Timing and follow-up</div><ul><li>Approach when you have <b>a new reason</b> — new footage, a credit, a showcase. "Checking in" is not a reason</li><li>Follow up <b>once</b>, a few weeks later, and only with something new. Then leave it. A no today is often a not-yet</li><li><b>A referral beats every cold approach.</b> A casting director, a teacher, or one of their current clients putting your name in the email is worth more than fifty perfect letters</li></ul>`+
 `<div class="hd">The line that is not negotiable</div><p><b>A real agent is paid a commission out of what you earn — usually around 10% — after you book.</b> Anyone asking for a signing fee, a registration fee, or insisting you shoot with their photographer or take their class first is not an agent. Walk, and do not argue about it.</p>`+
 `<p>The ${DIR}-company directory on CastSlate lists companies by tier with their submission preferences, so you can work through the ones that are actually open to you instead of guessing.</p>`,
 src:['card'],s:['How do I get an agent?','Am I ready for an agent?','Agent vs manager?']},
...
{id:'theater',k:'theater theatre stage acting play musical broadway equity aea open call ecc epa audition monologue',
 ...
 `<div class="hd">What to bring</div><ul><li>Two contrasting <b>monologues</b>, roughly a minute each, memorised cold and kept audition-ready year round &mdash; and lead with the <b>lighter</b> one, not the screaming one. Ask me about monologues and I will explain why</li><li>For musicals: <b>16 or 32 bars</b> in your best range, plus sheet music in your key, clearly marked, in a binder</li><li>Headshot and resume, stapled back to back, cut to headshot size</li></ul>`+
...
{id:'monologue',k:'monologue prepare choose piece audition speech one minute contrasting which monologue should i pick dramatic funny comedic first impression screaming crying',
 a:c=>`Two contrasting pieces, roughly a minute each, kept ready all year. Don't scramble for them the week you need them.`+
 `<div class="hd">The one thing to get right first</div><p><b>Do not make your main monologue the heaviest thing you own.</b> Not the screaming one, not the sobbing one, not the violent one. I know why actors reach for those — big emotion feels like proof of range, and suffering on stage feels powerful. For a <b>first impression</b> it backfires almost every time.</p>`+
 `<p>Think about the room from their side. A casting director has watched thousands of auditions, and a lot of today's have already been someone shouting or crying at them from four feet away. <b>Nobody enjoys being screamed at in a small room.</b> The tenth actor to break down in tears before lunch is not memorable, they are exhausting — and you get remembered as the person who made the room heavy.</p>`+
 `<div class="hd">Lead with something light</div><ul><li><b>Pick something funny.</b> Make them laugh and you are instantly the best two minutes of their morning. That is a memorable first impression, and it is far rarer than a good dramatic piece</li><li>Comedy is also harder, and they know it. Timing, listening and control read louder than volume ever does</li><li>Keep it <b>inside your real age range</b> and inside a character you could be cast as tomorrow</li><li>Keep the heavy piece as your <b>second</b>. That is what a contrasting pair is for — they will ask if they want it</li></ul>`+
 `<p>None of this says you cannot do drama. It says you do not open with it.</p>`+
 ...
 src:[],s:['What annoys casting directors?','How do I become a theater actor?','What should I wear to an audition?']},

{id:'monologueprep',k:'prepare monologue rehearse work the piece moment before who am i talking to cut it down off book place the partner eyeline time it practise preparing a monologue',
 a:c=>`Choosing the piece is one job; working it is another. This is the working part.`+
 `<div class="hd">Treat it as a scene, not a speech</div><ul><li><b>Who are you talking to?</b> A specific person, not the room. Name them in your head</li><li><b>What do you want from them?</b> Not "to express my grief" — something they can give you or refuse. The piece is you trying to get it</li><li><b>The moment before.</b> What happened ten seconds ago that made this come out of you now. If the answer is "nothing", the first line has nowhere to come from</li><li><b>Why today?</b> A speech that could have been said any time last year plays as recitation</li></ul>`+
 `<div class="hd">Cut it properly</div><ul><li>Sixty to ninety seconds. Cut narration and scene-setting, never the turn — the moment the tactic changes is the whole reason to do the piece</li><li>Cut so the ending is <b>an ending</b>. Fading out mid-thought reads as forgetting your lines</li><li><b>Time it out loud, standing up.</b> On the page everything is faster than it is in a room</li></ul>`+
 `<div class="hd">Get off book early, then stop rehearsing the reading</div><p>Being word-perfect is the floor, not the work. Once it is in your body, run it for what you <b>want</b>, not for how it sounded last time. A monologue rehearsed for its line readings arrives locked, and the first adjustment they give you takes it apart.</p>`+
 `<div class="hd">Where to put the other person</div><ul><li>In the room: <b>just off the panel</b>, at their eyeline height. Not at the casting director's face — you make them act, and they came to watch</li><li>On tape: <b>just beside the lens</b>, and hold it there. Eyes wandering around the room reads as unprepared</li><li>No chair standing in for a scene partner, no props you have to manage, no floor work. Stillness plays; choreography distracts</li></ul>`+
 `<p>Keep the contrasting second piece equally ready. If they ask for it, it is because the first one worked.</p>`,
 src:[],s:['Which monologue should I pick?','Where can I find monologue books?','How do I prepare for an audition?']},
...
{id:'reellength',k:'how long reel length showreel minutes seconds duration how long should my reel be how long do they watch one scene number of clips',
 a:c=>`Short answer: <b>they watch seconds, so make it short and put the best thing first.</b>`+
 `<div class="hd">How long they actually watch</div><p>Most reels are never finished by anyone. On a role with hundreds of submissions the person watching is deciding whether to keep going, not marking your work out of ten. Assume the ending will never be reached and build accordingly.</p>`+
 `<div class="hd">How long yours should be</div><ul><li><b>60 to 90 seconds</b> is plenty. Two minutes is the ceiling, not the target</li><li><b>Two to four clips</b>, one per look. A reel is not a filmography</li><li><b>One strong scene is a reel.</b> If you have exactly one good piece of footage, that is your reel — padding it out with weaker material lowers the average, and the average is what gets remembered</li><li>Anything longer — a full scene, a self-tape, a short film — lives on your profile <b>as its own clip</b>, where someone already interested can go and watch it</li></ul>`+
 `<div class="hd">What burns the time you have</div><ul><li>Stills, montages, music beds and credit sequences. All of it is time not spent watching you act</li><li>Clips where the viewer has to work out which one you are</li><li>Anything where the other actor has the scene</li></ul>`+
 `<p>Order it by what you would be cast as <b>tomorrow</b>, not by what you are proudest of. What the opening seconds specifically need to be is its own answer — ask me what makes casting skip a reel.</p>`,
 src:['profile'],s:['What makes casting skip a reel?','How do I build a reel with no footage?','What is the Slate Video?']},
...
{id:'cdpeeves',k:'annoy annoys annoying pisses off piss off irritate frustrate casting directors hate pet peeve pet peeves turn off turn offs empty profile no video one headshot incomplete lazy why do they pass what puts them off mistakes actors make',
 ...
 `<div class="hd">And in the room: the screaming monologue</div><p>This one is nearly universal and almost nobody warns beginners about it. <b>Do not lead with your heaviest piece.</b> Not the shouting one, not the sobbing one, not the violent one.</p>`+
 `<p>They have watched thousands of auditions, and several of today's have already been someone crying or yelling at them from four feet away. <b>Nobody enjoys being screamed at in a small room.</b> Actors reach for it because big emotion feels like proof of range — for a first impression it backfires, and you get remembered as the person who made the room heavy.</p>`+
 `<p><b>Pick something light instead. Make them laugh.</b> Comedy is harder, they know it is harder, and being the funny two minutes of a long morning is a far rarer thing to be. Keep it inside your real age range, and save the heavy piece for second — that is what a contrasting pair is for.</p>`+
...
{id:'studios',k:'studio studios production company companies netflix disney warner universal paramount sony a24 blumhouse lionsgate neon focus searchlight hbo max hulu apple amazon mgm streamer streamers network networks abc nbc cbs fox cw who makes movies major studio independent',
 ...
 src:[],s:['How does casting actually work?','How do I get an agent?']},
...
{id:'intimacycoord',k:'what does an intimacy coordinator do job role choreograph check in who hires the intimacy coordinator is there one on set coordinator responsibilities',
 a:c=>`An intimacy coordinator is a <b>department, not a chaperone</b>. Production hires them, the same way a production hires a stunt coordinator, and for the same reason: the scene involves bodies and it should be choreographed rather than improvised.`+
 `<div class="hd">Before the day</div><ul><li>They read the scene and the contract, then <b>talk to you privately</b> — what you have agreed to, what you have not, and where your no is. That conversation is not a negotiation and nobody from production is in it</li><li>They translate the director's intention into <b>specific, agreed, repeatable actions</b>: this hand, here, for this long. The director says what the scene needs; the IC works out what will actually be done</li><li>They sort the practical kit — modesty garments, barriers, robes — and confirm the agreed coverage and framing</li></ul>`+
 `<div class="hd">On the day</div><ul><li>They <b>choreograph it like a fight</b> and rehearse it clothed. Nothing gets invented once the camera rolls</li><li>They run a check-in before the first take and after the last one, and they watch the monitor for the take drifting away from what was agreed</li><li>They keep a <b>closed set</b> actually closed, and they are the person who says "that was not what we set" out loud so you do not have to</li></ul>`+
 `<div class="hd">What they are not</div>`+`<ul><li>Not your agent, and not a censor. A well-run intimate scene is usually <b>bolder</b> than an awkward one, because everyone knows the plan</li><li>Not a delay. Choreographed scenes shoot faster than scenes being negotiated between takes</li><li>Not automatic on every production. If there is no IC on a job with intimate content, <b>ask why</b> — and ask for the scene to be agreed in writing and rehearsed before you shoot it. Being the person who asks is normal and professional</li></ul>`+
 `<p>Your rights around nudity and simulated intimacy — riders, consent, what you can refuse and when — is the other half of this. Ask me about your rights on an intimacy scene.</p>`,
 src:['safety'],s:['What are my rights on an intimacy scene?','What is set etiquette?','What should I check before signing a contract?']},
...
C('continuity','Camera','continuity matching action repeat takes coverage consistency match the take',{
...
C('samesame','Psychology','same performance every time same every take look the same cant vary stuck repeating one note range sameness samey',{
...
function compose(c,depth,level,shape){
  ...
  /* quick */
  const why1=c.why.charAt(0).toUpperCase()+c.why.slice(1);   /* it follows a full stop here, not a colon */
  return [{b:`<p>${open}</p><p><b>${c.what.charAt(0).toUpperCase()+c.what.slice(1)}.</b> ${why1}.</p>`+
...
/* Words that are English before they are jargon. A key made only of these can
   never claim a question on its own. */
const GLOSS_WEAK=new Set(('the a an and or of to for with on in at is are was were be'+
 ' it its this that these those my your our their you i we they he she'+
 ' what which who how why when where do does did can could would should'+
 ' one two three first second next last all any some more most other'+
 ' up down out off over under back front here there now then'+
 ' get got go going make made take taken put set see saw look'+
 ' good bad big small long short new old real full half'+
 ' time day week month year thing things people person'+
 ' shot shoot show room call calls line lines part parts work working'+
 ' pay paid cost costs price prices free').split(' '));
function glossaryFor(q){
  const low=' '+q.toLowerCase().replace(/[^a-z0-9\s]/g,' ').replace(/\s+/g,' ')+' ';
  let hit=null,best=0;
  Object.entries(GLOSSARY).forEach(([term,g])=>{
    /* the term itself, as a phrase, is the strongest possible signal */
    if(term.length>2&&low.includes(' '+term+' ')){
      const w=term.length+40;
      if(w>best){hit={term,g};best=w;}
    }
    g.k.split(/\s*\|\s*/).forEach(k=>{
      k=k.trim(); if(!k)return;
      const words=k.split(' ').filter(Boolean);
      words.forEach(word=>{
        if(word.length<=2||GLOSS_WEAK.has(word))return;
        if(!low.includes(' '+word+' '))return;
        const w=word.length;
        if(w>best){hit={term,g};best=w;}
      });
      if(words.length>1&&low.includes(' '+k+' ')){
        const w=k.length+20;
        if(w>best){hit={term,g};best=w;}
      }
    });
...
const LESSON_INTENT=[
 ...
 /* "How do I take an adjustment?" used to be caught here and answered with the
    ...
    lesson". It goes to the taking-direction concept instead; the lesson is
    still what "teach me direction" opens. */
 [/\b(audition|room) (lesson|exercise)\b|\bfirst impression (in|at) (an )?audition\b|\brecover(ing)? (after|from) a mistake\b|\bstop trying to impress\b/i,'roomnerves'],
...
/* ── retrieval ───────────────────────────────────────────────── */
/* Ordinary connectives must never decide an answer. "How do I match my action
   between takes" was answered with cancel-vs-delete, because that entry's
   keywords contain the word "between" and nothing else in the question scored.
   Same failure mode as the glossary matching on "the". */
const STOP=new Set(("a an the is are was were be been do does did i im me my mine you your yours it its of to for on in at and or but if so how what when where which who whom that this these those can could would should will shall may might must have has had get got just about with as not no yes please thanks thank hey hi hello there here from than then them they he she we us our there's whats"+
 " between difference differences into each every both again also very only much many way ways thing things actually really own another others something anything everything nothing someone anyone everyone okay ok need needs want wants know help best better worse still").split(' '));
...
/* Words the corrector must never touch. Its vocabulary is acting jargon, so
   any real word missing from it gets dragged onto the nearest term — and at
   edit-distance 1 that rewrites the whole message. Two live examples:
   "fuck" became "luck" and returned the did-not-hear-back answer, "shit"
   became "shot" and opened the shot-size lesson. These are real words, not
   typos. They are excluded from correction, and are never targets either
   (they are not in VOCAB), so "fick" cannot be corrected onto them. */
const NOSPELL=new Set(('fuck fucks fucking fucked fuckin fuk fuckoff shit shite shitty shits crap crappy '
 +'damn damned dammit goddamn hell heck bloody bollocks bugger buggered wank wanker arse arsehole '
 +'ass asshole jerk bitch bastard piss pissed prick twat cunt bullshit horseshit dogshit '
 +'wtf omfg fml ffs ugh argh aargh ouch oops yikes meh nope nah yeah yep yup okay '
 +'hate hates hated love loves loved like likes liked want wants wanted tired sick done '
 +'quit quits quitting sorry thanks thank please cool nice great awful awesome stupid dumb '
 +'sucks suck sucked worst worse better best fine good bad sad mad angry upset broke broken '
 +'money rent food sleep sleeping eat eating drunk high weed drugs sex porn kill dead die dying'
...
function despell(text){
  ...
  const out=text.split(/(\s+)/).map(tok=>{
    ...
    if(w.length<4||VOCAB.has(w)||STOP.has(w)||NOSPELL.has(w))return tok;
...
const INTENT=[
 /* ── One question, one answer ───────────────────────────────────────────
    Every rule in this block used to lose its question to a broader entry
    further down, which handed back an answer written for a DIFFERENT
    question — "How do I build outside LA and NY?" returned the
    do-I-have-to-move-to-LA answer word for word. Each one now goes to the
    source that actually answers it: its own concept, its own glossary term,
    or its own entry. Narrow on purpose, and first on purpose. */
 [/\b(build|building|make it|making it|work|working|career|based)\b[^?.!]{0,24}\b(outside|away from|not in|beyond|far from)\b[^?.!]{0,18}\b(la|l\.a\.|los angeles|new york|ny|nyc)\b|\boutside la and ny\b|\bregional (market|actor|career)\b|\bsmall(er)? market\b/i,'c:regional'],
 [/\bcheat(ing)? out\b/i,'c:cheatout'],
 [/\bupstag(e|ed|ing)\b/i,'c:upstaging'],
 [/\btech week\b|\bcue.?to.?cue\b/i,'g:tech week'],
 [/\bspike marks?\b/i,'g:spike mark'],
 [/\bback to one\b/i,'g:back to one'],
 [/\bwhat('?s| is| are)\b[^?.!]{0,12}\bbuyouts?\b|\bbuyouts? mean\b/i,'g:buyout'],
 [/\bwhat('?s| is| are)\b[^?.!]{0,10}\bblocking\b|\bblocking mean\b/i,'g:blocking'],
 [/\bplaying age\b/i,'c:agerange'],
 [/\btake (an|a|the) adjustment\b|\bgiven an adjustment\b|\btaking direction\b|\bthey give me a note\b|\bredirect(ed|ion)\b[^?.!]{0,18}\b(audition|room|take)\b/i,'c:direction'],
 [/\b(stop|less|reduce|calm|handle|deal with|get over)\b[^?.!]{0,18}\b(nervous|nerves|anxious|anxiety|stage fright)\b|\bi get nervous\b|\baudition nerves\b|\bstop being nervous\b/i,'c:nerves'],
 [/\bam i ready for (an )?(agent|representation)\b|\bready for an agent\b|\bready for representation\b/i,'c:readyforagent'],
 [/\bhow do i approach\b[^?.!]{0,14}\b(agency|agencies|agent|agents)\b|\bapproach(ing)? (an |the )?(agency|agencies)\b|\bsubmit(ting)? to (an |the )?(agency|agencies|agents)\b|\bquery letter\b|\bmail(ing)? (to )?(agents|agencies)\b/i,'approachagency'],
 [/\bhow (do|should) i (prepare|prep|work|work on|rehearse|learn)\b[^?.!]{0,14}\bmonologue\b|\bprepar(e|ing) (a|my|the) monologue\b|\bwork(ing)? on (a|my|the) monologue\b|\brehears(e|ing) (a|my|the) monologue\b/i,'monologueprep'],
 [/\bhow long\b[^?.!]{0,30}\bwatch(ing)?\b[^?.!]{0,18}\b(reel|reels|showreel|tape)\b|\bhow long should (my|a|the) (reel|showreel)\b|\breel length\b|\bhow long (is|should) a (reel|showreel)\b/i,'reellength'],
 [/\bdo i (need|have) to join\b[^?.!]{0,20}\b(union|sag|sag.?aftra|equity|aea)\b|\bshould i join\b[^?.!]{0,20}\b(union|sag|sag.?aftra|equity|aea)\b|\bwhen should i join (the )?(union|sag)\b|\bhow do i (become|get) (sag|union) eligible\b|\b(sag|union) eligib(le|ility)\b/i,'joinunion'],
 [/\bwhat does (an?|the) intimacy coordinator\b|\bintimacy coordinator'?s? (job|role)\b|\bwho is the intimacy coordinator\b|\bdoes the intimacy coordinator\b/i,'intimacycoord'],
 ...
 [/\bis (it|this|that|premium|castslate|the membership|a membership|the subscription|a subscription|paying|going premium|upgrading)\b[^?.!]{0,14}\bworth\b|\bworth the money\b|\bworth (it )?(paying|upgrading|subscribing|going premium)\b|\bshould i (pay|upgrade|subscribe|go premium)\b|\bwhy (should i )?pay\b|\bconvince me\b|\bis premium worth\b/i,'worth'],
 /* Everything else that gets asked as "is it worth it" keeps its own answer. */
 [/\b(background|extra|extras|bg) work\b|\bwork(ing)? as an extra\b|\bbe an extra\b|\bbackground actor\b|\bis being an extra\b/i,'extras'],
 [/\bwhat happens (after|once|when) i (submit|apply|send)\b|\bafter i submit(ted)?\b|\bwhat happens next\b|\bhow long (do i wait|until i hear)\b/i,'callback'],
 [/\b(am i|are we|when am i) ready for (an )?(agent|representation)\b|\bready for an agent\b|\bready for representation\b/i,'agent'],
 [/\bplaying age\b|\bwhat age (do i|am i) (read|play|look)\b|\bwhat'?s my age range\b|\bmy age range\b/i,'type'],
 ...
 /* Cost gets asked a dozen ways and it always means the plans. Kept narrow
    enough that "how much do actors get paid" still reaches the rates answer. */
 [/\b(what'?s|whats|what is|whats)\s+(the\s+)?(cost|price|fee|damage|rate)\b|\bwhat (does|do|would) (it|this|that|castslate|premium|a membership|the membership|membership|a subscription) cost\b|\bhow much (is|are|would) (it|this|that|premium|castslate|a |the )?(membership|subscription|plan|account)?\b(?![^?.!]{0,24}\b(actor|actors|performer|extra|background|day rate|sag|union|headshot|photographer|class|classes|coach)\b)|\bhow much\b[^?.!]{0,20}\b(premium|membership|subscription|per month|a month|monthly|per year|a year|yearly|annually|to (join|upgrade|subscribe|go premium))\b|\bwhat do (you|they) charge\b|\bhow much does (it|this|castslate|premium|the membership) cost\b|^\s*(the )?(cost|costs|price|pricing|prices|fees?)\s*\??$|\bcost of (premium|membership|a subscription)\b|\bis (it|castslate|premium) (free|paid|expensive)\b|\bhow much to (join|sign up|use it|upgrade|go premium)\b/i,'pricing'],
 ...
 /* ⚠️ No agency is ever named, big or small (owner, 2026-08-25). Questions that
    fish for names get the tiers-and-how-to-approach answer instead. */
 [/\b(caa|wme|uta|gersh|paradigm|icm|buchwald|innovative artists|a3 artists|abrams artists|cesd|kmr)\b|\b(biggest|top|best|major|which|what) (talent )?(agenc(y|ies)|agents)\b|\bagency tiers?\b|\bbig (three|four) agenc|\bname (some |a few |me )?agenc(y|ies)\b|\blist of agenc/i,'dir'],
...
/* ── layer 0: distress. Runs before EVERYTHING — before spell-correction,
   small talk and keyword scoring — because those will happily answer a
   sentence like "I can't do this anymore" with the subscription-cancellation
   entry, and "I want to kill myself" with the one about money. They did.
   Nothing below this line is allowed to answer these messages. */
const CRISIS=/\b(kill(ing)? myself|end(ing)? my life|take my own life|want(ing)? to die|wanna die|better off dead|suicidal|suicide|hurt(ing)? myself|self[ -]?harm|cut(ting)? myself|no reason to live|don'?t want to be here|can'?t go on|cant go on)\b/i;
/* First-person framing, tightly bound: an early looser version caught "how do
   I play a hopeless character", which is a craft question and must still reach
   the concept graph. The feeling word has to attach to the speaker or to the
   situation, not merely appear in the same sentence. */
const DISTRESS=/\b(i'?m|im|i am|i feel|i'?m feeling|feeling)\W+(so |really |very |completely |totally |just |kinda |kind of )*(hopeless|worthless|depressed|miserable|broken|crushed|defeated|useless|a failure|failing|lost|burnt out|burned out|crying|breaking down|falling apart)\b|\b(this|it|everything|all of this)\W+(is|feels)\W+(so |really |just )?(hopeless|pointless|useless|impossible|too much|not worth it)\b|^\W*(i give up|i quit|i'?m done|im done|i can'?t do this( anymore)?|i cant do this( anymore)?|i cannot do this( anymore)?|nobody wants me|i hate (this|it|myself|my life|everything)|i want to give up|whats the point|what'?s the point)\W*$/i;

function think(text,c){
  if(CRISIS.test(text)) return {crisis:true};
  if(DISTRESS.test(text)) return {distress:true};
  ...
  for(const [re,id] of INTENT) if(re.test(text)){
    /* A route can point at a concept ("c:regional") or a glossary term
       ("g:spike mark") as well as a canned KB entry. Questions that used to
       share one entry are sent to the source that answers THEM — see the
       one-question-one-answer block at the top of INTENT. */
    if(id.slice(0,2)==='c:'){
      const cc=CONCEPTS.find(x=>x.id===id.slice(2));
      /* "quick" on purpose: these are questions, not requests to be taught, so
         they get answered in one message with an offer to go deeper — an
         explicit "teach me X" is caught above this loop and still opens the
         full lesson. */
      if(cc) return {teach:{c:cc,depth:'quick',shape:shapeOf(text)}};
    }else if(id.slice(0,2)==='g:'){
      const gg=GLOSSARY[id.slice(2)];
      if(gg) return {gloss:{term:id.slice(2),g:gg},shape:'vocab'};
    }else{
      const e=KB.find(x=>x.id===id); if(e) return {entry:e};
    }
  }

  /* ⚠️ Spell-correction sits HERE: after the exact layers, before the fuzzy
     ones. It used to run straight after DECLINE, which gave it first refusal
     over everything — and because its vocabulary is built from acting jargon,
     any ordinary word missing from it was rewritten into a similar-looking
     term. "Who are the biggest talent agencies?" became "the DIGEST talent
     agencies" and got a different answer entirely, even though the original
     matched an intent rule exactly. Moving it to the very end was worse the
     other way: a typo would lose to a weak keyword score and never get
     corrected at all. Exact matches beat it; fuzzy scoring does not. */
  if(!c||!c.__retry){
...
const TOPIC_POOL=[
 ...
 'How do I get an agent?','Agent vs manager?',
...
/* The first screen is the only one everybody sees, so it is chosen rather than
   filled from the pool. Craft and profile questions first — the free teaching
   is what earns the right to sell anything — then the two questions that lead
   to what Premium actually solves (the agency directory, unlimited video and
   submissions), then price last. A Premium member never sees the price pair. */
const OPENING_FREE=[
 'Teach me the fundamentals',
 'What makes casting skip a reel?',
 'What annoys casting directors?',
 'How do I improve my profile?',
 'Which headshot should I lead with?',
 'How do I build a reel with no footage?',
 'What should I submit to?',
 'Why is nobody getting back to me?',
 'What is my type?',
 'How do I get an agent?',
 'What are my rights on an intimacy scene?',
 'Do I need to join SAG-AFTRA?',
 'Do I need to move to LA?',
 'What does Premium include?',
 'What does it cost?'
];
const OPENING_PREMIUM=[
 'Teach me the fundamentals',
 'What makes casting skip a reel?',
 'What annoys casting directors?',
 'How do I improve my profile?',
 ...
 'Tell me about the agency directory'
];
let poolAt=0;
...
function suggPlus(list,cap){
  ...
  /* Stride rather than slice. A contiguous run came back as eleven craft
     lessons in a row; stepping through the pool spreads the order across
...
/* ── thread scrolling ──────────────────────────────────────────
   Every answer used to be slammed to the bottom, which on a long one dumps
   you at the END of text you have not read yet. Three rules now:
   1. the move is eased, never a jump;
   2. a message TALLER than the visible thread anchors its own top instead,
      so you start reading at the first line;
   3. nothing moves at all if you had scrolled up to re-read something —
      being yanked away mid-sentence is the thing that actually annoys people.
   The tween is manual with a guaranteed final assignment, because native
   smooth behaviour and requestAnimationFrame can both silently no-op here. */
const NEAR_BOTTOM=140;
let scrollRAF=null, scrollFallback=null;
...
function atBottom(){
  const t=$('thread'); if(!t)return true;
  return (t.scrollHeight-t.scrollTop-t.clientHeight)<NEAR_BOTTOM;
}
function stopScroll(){
  if(scrollRAF){cancelAnimationFrame(scrollRAF);scrollRAF=null;}
  if(scrollFallback){clearTimeout(scrollFallback);scrollFallback=null;}
}
function glideTo(target,dur){
  ...
  const d=dur||Math.max(240,Math.min(560,Math.abs(to-from)*1.1));
  if(reducedMotion()||Math.abs(to-from)<3||typeof requestAnimationFrame!=='function'){t.scrollTop=to;refreshThreadEdge();return;}
  const t0=(window.performance&&performance.now)?performance.now():Date.now();
  const step=now=>{
    const k=Math.min(1,((now||Date.now())-t0)/d);
    t.scrollTop=from+(to-from)*(1-Math.pow(1-k,3));
    if(k<1){scrollRAF=requestAnimationFrame(step);}
    else{t.scrollTop=to;stopScroll();refreshThreadEdge();}
  };
  scrollRAF=requestAnimationFrame(step);
  /* If rAF never runs (throttled tab), land it anyway rather than sit still. */
  scrollFallback=setTimeout(()=>{stopScroll();t.scrollTop=to;refreshThreadEdge();},d+300);
}
/* force: the visitor's own message and the greeting always come into view. */
function bottom(force){
  const t=$('thread'); if(!t)return;
  if(!force&&!atBottom())return;
  glideTo(t.scrollHeight);
}
function offsetInThread(el){
  const t=$('thread');
  return el.getBoundingClientRect().top-t.getBoundingClientRect().top+t.scrollTop;
}
/* Bring a freshly added row into view the way a reader would want it.
   `follow` is measured BEFORE the row is appended — asking atBottom() after
   the fact always answers "no", because the new content is what pushed you up. */
function reveal(row,follow){
  const t=$('thread'); if(!t||!row||!follow)return;
  const tall=row.offsetHeight>(t.clientHeight-56);
...
function msg(dir,html){
  ...
  row.appendChild(b);th.appendChild(row);
  reveal(row,stick||dir==='out');
  return row;
}
function outMsg(text){
  ...
  row.after(r);st.lastOut=r;
  bottom(true);   /* you always follow your own message down */
}
...
function typing(){
  ...
  if(stick)bottom(true);   /* the dots are short, and they say "wait here" */
  return row;
}
function card(o){
  ...
  const stick=atBottom();
  $('thread').appendChild(el);bottom(stick);
}
function srcs(ids){
  ...
  const stick=atBottom();
  $('thread').appendChild(w);bottom(stick);
}
/* "Is there anything below?" — not "are we at the top?". The hint used to
   vanish on the first scroll while two more rows were still hidden. Kept out
   of sugg() so the scroll button can re-run it without waiting for an event. */
/* Softens the thread's bottom edge only while content is hidden below it. */
function refreshThreadEdge(){
  const t=$('thread'), wrap=$('suggwrap');
  if(!t||!wrap)return;
  wrap.classList.toggle('spill',(t.scrollTop+t.clientHeight)<(t.scrollHeight-6));
}
function refreshSuggHint(){
  const w=$('sugg'), wrap=$('suggwrap');
  if(!w||!wrap)return;
  refreshThreadEdge();
  const queued=!!(st.moreQueue&&st.moreQueue.length);
  const scrolls=queued||w.scrollHeight>w.clientHeight+4;
  const atEnd=!queued&&(w.scrollTop+w.clientHeight)>=(w.scrollHeight-4);
  wrap.classList.toggle('scrolls',scrolls);
  wrap.classList.toggle('atend',atEnd);
  wrap.classList.toggle('folded',!!st.suggFolded);
  const fold=$('suggfold'), lbl=$('suggfoldlbl');
  if(fold&&lbl){
    const n=w.children.length+(st.moreQueue?st.moreQueue.length:0);
    lbl.textContent=st.suggFolded?('Show questions'+(n?' ('+n+')':'')):'Hide';
    fold.setAttribute('aria-expanded',st.suggFolded?'false':'true');
    fold.setAttribute('aria-label',st.suggFolded?'Show suggested questions':'Hide suggested questions');
  }
}
/* Fold the chip block away so the answer has the panel to itself, and put it
   back on the next press. The choice sticks for the rest of the conversation —
   somebody who folded it once wants to read, not re-fold after every reply. */
function toggleSuggFold(){
  st.suggFolded=!st.suggFolded;
  refreshSuggHint();
  if(!st.suggFolded){const w=$('sugg');if(w)w.scrollTop=0;}
  bottom();
}
/* ⚠️ iOS swallows the CLICK that follows a scroll, a flick's momentum, or any
   repaint underneath the finger — which is why a chip used to need two or three
   presses. Activation moved onto pointerup/touchend, which fires on the first
   press every time; the synthesised click that arrives afterwards is ignored
   for 700ms so nothing fires twice. Movement over 10px means the finger was
   scrolling, not pressing, and nothing fires at all. */
function tapBind(el,fn){
  if(!el)return;
  let sx=0,sy=0,t0=0,moved=false,fired=0;
  const down=e=>{sx=e.clientX||0;sy=e.clientY||0;t0=Date.now();moved=false;};
  const move=e=>{if(Math.abs((e.clientX||0)-sx)>10||Math.abs((e.clientY||0)-sy)>10)moved=true;};
  const up=e=>{
    if(moved||!t0||Date.now()-t0>900)return;
    t0=0; fired=Date.now(); fn(e);
  };
  try{
  ...
  /* Mouse, keyboard (Enter/Space on a focused button) and any browser without
...
function suggAppend(list){
  const w=$('sugg');
  (list||[]).forEach(l=>w.appendChild(suggBtn(l)));
  refreshSuggHint();
}
function sugg(list){
  const w=$('sugg');w.innerHTML='';w.scrollTop=0;st.moreQueue=[];
  const wrap0=$('suggwrap');
  if(wrap0)wrap0.classList.toggle('has',!!(list&&list.length));
  if(wrap0)wrap0.classList.remove('scrolls','atend');
  if(wrap0)wrap0.classList.toggle('folded',!!st.suggFolded);
  (list||[]).forEach(l=>w.appendChild(suggBtn(l)));
...
/* While the full-height sheet is open on a phone the page behind it must not
   scroll, or a flick that overshoots the thread drags the site instead. */
function lockPage(on){
  try{
    const d=document.documentElement,b=document.body;
    if(on){ if(st._lock)return; st._lock={d:d.style.overflow,b:b.style.overflow};
            d.style.overflow='hidden'; b.style.overflow='hidden'; }
    else if(st._lock){ d.style.overflow=st._lock.d; b.style.overflow=st._lock.b; st._lock=null; }
  }catch(_){}
}
/* The nudge fires on every page load, five seconds in — never once-per-visitor
   (owner's call), and never while the panel is already open. */
function hideNudge(){
  const n=$('nudge'); if(!n||!n.classList.contains('on'))return;
  n.classList.add('out');
  setTimeout(()=>{n.classList.remove('on');n.classList.remove('out');},320);
}
function armNudge(){
...
function hello(){
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      /* The curated fifteen are shown in full, and the rest of the pool is
...
function send(text){
  ...
  if(!st.lesson&&st.paused){
    ...
    if(RESUME.test(p)||YES.test(p)){
      ...
      const t0=typing();
      setTimeout(()=>{t0.remove();markRead();lessonStep();},THINK());
      return;
    }
  ...
  setTimeout(()=>{ t.remove();markRead(); respond(r,c); },THINK());
}
...
function respond(r,c){
    /* No lesson, no upsell, no suggestion chips steering back to acting. */
    if(r.crisis){
      st.misses=0;st.lesson=null;st.paused=null;
      msg('in',`I'm glad you told me, and I'm not going to answer that with an acting lesson.`);
      setTimeout(()=>{
        msg('in',`If you're in immediate danger, please call your local emergency number. In the US you can call or text <b>988</b> (Suicide &amp; Crisis Lifeline) any time, day or night. Anywhere else, <b>findahelpline.com</b> lists free, confidential services by country.`);
        setTimeout(()=>{
          msg('in',`I'm an automated assistant, so I'm not the right thing to lean on here — but I can pass you to a person on the CastSlate team if that would help.`);
          sugg(['Talk to a human']);
        },700);
      },700);
      return;
    }
    if(r.distress){
      st.misses=0;
      msg('in',`That sounds genuinely hard — and it is not a stupid thing to feel. This business hands out a lot of silence and almost no explanation, and it wears people down.`);
      setTimeout(()=>{
        msg('in',`Tell me what is actually going on and I will give you something concrete rather than a pep talk. Or I can put you through to a person.`);
        suggPlus(['Talk to a human','Why is nobody getting back to me?','How do I improve my profile?']);
      },700);
      return;
    }
    if(r.lesson){
...
/* ── composer tools ────────────────────────────────────────────────
   Four controls, and every one of them does something real. A button
   that looks live and isn't is worse than no button — the same reason
   the fake unread badge came off the launcher.                      */
const EMOJI=('\u{1F44B} \u{1F642} \u{1F603} \u{1F602} \u{1F609} \u{1F60E} \u{1F914} \u{1F62C} '
 +'\u{1F625} \u{1F644} \u{1F615} \u{1F621} \u{1F62D} \u{1F631} \u{1F44D} \u{1F44E} '
 +'\u{1F64C} \u{1F64F} \u{1F4AA} \u{1F91E} \u{2764}\u{FE0F} \u{2728} \u{1F525} \u{1F389} '
 +'\u{1F3AC} \u{1F3AD} \u{1F3A5} \u{1F3A4} \u{1F4F8} \u{1F4DD} \u{1F4C5} \u{2705}').split(' ');
...
/* Dictation is real, via the browser's own speech recognition. Where the
   browser has none the button is removed rather than left as decoration. */
function speechSupported(){
  return !!(window.SpeechRecognition||window.webkitSpeechRecognition);
}
function makeDictation(input,btn){
  const SR=window.SpeechRecognition||window.webkitSpeechRecognition;
  if(!SR){btn.remove();return;}
  let rec=null,live=false,base='';
  ...
  btn.onclick=()=>{
    ...
    try{
      ...
      rec.onstart=()=>{live=true;btn.classList.add('on');btn.setAttribute('aria-pressed','true');};
      rec.onresult=ev=>{
        let txt='';
      ...
      rec.onerror=()=>{live=false;btn.classList.remove('on');btn.setAttribute('aria-pressed','false');};
      rec.onend=()=>{live=false;btn.classList.remove('on');btn.setAttribute('aria-pressed','false');input.focus();};
      rec.start();
    }catch(_){btn.classList.remove('on');}
  };
}

function wireTools(){
  const input=$('input'), pop=$('emoji');
  /* emoji */
  if(pop&&!pop.childElementCount){
    EMOJI.forEach(ch=>{
      const b=document.createElement('button');
      b.type='button'; b.textContent=ch; b.setAttribute('aria-label','Insert '+ch);
      b.onclick=()=>{insertAtCursor(input,ch);};
      pop.appendChild(b);
    });
  }
  const tE=$('tEmoji');
  if(tE)tE.onclick=()=>{
    ...
    tE.setAttribute('aria-pressed',on?'true':'false');
    if(on)input.focus();
  };
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&pop&&pop.classList.contains('on')){pop.classList.remove('on');tE&&tE.setAttribute('aria-pressed','false');}});
  /* the other three go straight into the conversation */
  ...
  const th=$('thread');
  if(th)th.onscroll=refreshThreadEdge;

  tapBind($('sugg'),askChip);
  tapBind($('suggfold'),toggleSuggFold);

  const tM2=$('suggmore');
  if(tM2)tapBind(tM2,function(){
    /* Hand out the next batch before scrolling, so the button keeps revealing
       new questions all the way to the end of the pool instead of stopping
       at whatever happened to be rendered. A phone shows the strip at ~38vh,
       roughly twice the desktop height, so a desktop-sized batch of 8 ran the
       queue down in a handful of presses and looked like the list had ended —
       the batch is measured against what the strip can actually show. */
    const per=(typeof window!=='undefined'&&window.innerWidth<=560)?14:8;
    if(st.moreQueue&&st.moreQueue.length)suggAppend(st.moreQueue.splice(0,per));
    /* Set it, then ask for smooth. Native smooth scrolling has a history of
       no-oping in this codebase and a rAF tween is throttled in a background
       tab — either can silently do nothing, so the assignment goes first and
       the animation is only a nicety on top. */
    const w=$('sugg');
    const to=Math.min(w.scrollHeight-w.clientHeight, w.scrollTop+w.clientHeight-24);
    const from=w.scrollTop;
    w.scrollTop=to;
    try{
      if(!(window.matchMedia&&window.matchMedia('(prefers-reduced-motion:reduce)').matches)){
        w.scrollTop=from; w.scrollTo({top:to,behavior:'smooth'});
        setTimeout(()=>{ if(Math.abs(w.scrollTop-to)>2)w.scrollTop=to; refreshSuggHint(); },340);
      }
    }catch(_){ w.scrollTop=to; }
    refreshSuggHint();
  });
  ...
  if(tX)tX.onclick=function(){
    const p=$('panel'), small=p.classList.toggle('compact');
    tX.setAttribute('aria-label',small?'Make the panel bigger':'Make the panel smaller');
    tX.setAttribute('title',small?'Bigger':'Smaller');
    bottom();
  };
  armNudge();
}
