...
function glideTo(target,dur){
  ...
  if(reducedMotion()||Math.abs(to-from)<3||typeof requestAnimationFrame!=='function'){t.scrollTop=to;return;}
  ...
  const step=now=>{
    ...
    else{t.scrollTop=to;stopScroll();}
  };
  scrollRAF=requestAnimationFrame(step);
  /* If rAF never runs (throttled tab), land it anyway rather than sit still. */
  scrollFallback=setTimeout(()=>{stopScroll();t.scrollTop=to;},d+300);
}
...
function refreshSuggHint(){
  ...
  if(!w)return;
  const more=w.scrollHeight>w.clientHeight+4&&(w.scrollTop+w.clientHeight)<(w.scrollHeight-4);
  w.classList.toggle('more',more);
  if(wrap)wrap.classList.toggle('more',more);
}
function sugg(list){
  const w=$('sugg');w.innerHTML='';w.classList.remove('more');w.scrollTop=0;
  const wrap0=$('suggwrap'); if(wrap0)wrap0.classList.remove('more');
