...
module.exports=function register({check,addBoard,sentences,clean,minAge,maxAge,isGroup,famOf,srcNow,parseRoleRate,ctxNow}){
  ...
  const slotOf=(L,r)=>{const raw=(L._raw._roles||[]).find(x=>x.name===r.name||x._person===r.name||String(x.name).toUpperCase()===r.name);return String((raw&&raw._slot)||"");};
  ...
  check(8,"cast_count_vs_list","'N actors to play A and B' lists fewer parts than N",L=>{const m=String(L.synopsis).match(/looking for (\w+) [a-z ]+ to play ([^.]+)\./i);if(!m)return[];const W={one:1,two:2,three:3,four:4,five:5,six:6,seven:7,eight:8};const n=W[m[1].toLowerCase()]||0;const listed=m[2].split(/, | and /).length;return !/more$/.test(m[2])&&n&&listed<n?[{detail:m[0]}]:[];});
